...
SAML_CERT_PATH=$CERTS_PATH/saml/
TPM_CERT_PATH=$CERTS_PATH/tpm/

for directory in $BIN_PATH $LOG_PATH $CONFIG_PATH $CERTS_PATH $CERTDIR_TRUSTEDCAS $CERTDIR_TRUSTEDJWTCERTS $KEYS_PATH $KEYS_TRANSFER_POLICY_PATH $SAML_CERT_PATH $TPM_CERT_PATH; do
    mkdir -p $directory
    if [ $? -ne 0 ]; then
