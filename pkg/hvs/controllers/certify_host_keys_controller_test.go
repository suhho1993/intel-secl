...
var _ = Describe("CertifyHostKeysController", func() {
	...
	BeforeEach(func() {
		...
		caKey, _ := crypt.GetPrivateKeyFromPKCS8File(caKeyPath)
		caCert, _ := crypt.GetCertFromPemFile(caCertPath)
		// Generate aik certificate
		var err error
		aikcert, err = certifyHostAiksController.CertifyAik(&aikPubKey, aikName, caKey.(*rsa.PrivateKey), caCert, constants.AIKCertValidity)
	...
	Describe("Create Binding key certificate", func() {
		Context("Provide valid data in request", func() {
			It("Return Binding key certificate", func() {
				...
				pCAFileStore = controllers.NewPrivacyCAFileStore(caKeyPath, caCertPath, "","")
				certifyHostKeysController = &controllers.CertifyHostKeysController{Store: pCAFileStore}
